...
class JsonDatabase {
  ...
  perkaraFile = path.join(DATA_DIR, 'perkara.json');
  disposisiFile = path.join(DATA_DIR, 'disposisi.json');
  constructor() {
    this.init();
  }
  init() {
...
try {
  ...
  if (!checkAdmin) {
    sqlite.prepare('INSERT INTO users (nama, email, pass, role, createdAt) VALUES (?, ?, ?, ?, ?)').run('Admin System', 'admin', 'admin', 'Admin', new Date().toISOString());
  }
  class SqliteDatabase {
    getUsers() {
      return sqlite.prepare('SELECT id, nama, email, pass, role, createdAt FROM users').all();
    }
    getUserByEmail(email) {
      return sqlite.prepare('SELECT id, nama, email, pass, role, createdAt FROM users WHERE lower(email) = ?').get(email.toLowerCase());
    }
    createUser(user) {
      const info = sqlite.prepare('INSERT INTO users (nama, email, pass, role, createdAt) VALUES (?, ?, ?, ?, ?)').run(user.nama, user.email, user.pass, user.role, new Date().toISOString());
      return {
        ...user,
    ...
    getAllPerkara() {
      const rows = sqlite.prepare('SELECT id, data FROM perkara ORDER BY id ASC').all();
      return rows.map(r => {
        try {
    ...
    getPerkaraById(id) {
      const row = sqlite.prepare('SELECT id, data FROM perkara WHERE id = ?').get(id);
    ...
    savePerkara(rec, index) {
      ...
      if (typeof index === 'number' && index >= 0 && index < list.length) {
        ...
        sqlite.prepare('UPDATE perkara SET data = ?, updatedAt = ? WHERE id = ?').run(JSON.stringify(updated), now, target.id);
      } else {
        const info = sqlite.prepare('INSERT INTO perkara (data, createdAt, updatedAt) VALUES (?, ?, ?)').run(JSON.stringify(rec), now, now);
    ...
    getDisposisi() {
      const rows = sqlite.prepare('SELECT id, perkara_id, data, createdAt FROM disposisi').all();
      return rows.map(r => {
        try {
          const parsed = JSON.parse(r.data);
          parsed.id = r.id;
    ...
    saveDisposisi(disp) {
      const now = new Date().toISOString();
      const info = sqlite.prepare('INSERT INTO disposisi (perkara_id, data, createdAt) VALUES (?, ?, ?)').run(disp.perkaraId || 0, JSON.stringify(disp), now);
      return {
        ...disp,
        id: Number(info.lastInsertRowid)
      };
