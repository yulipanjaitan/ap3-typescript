...
export function terbilangHuruf(angka) {
    ...
    if (n < 12)
        return bilangan[n];
    if (n < 20)
        return terbilangHuruf(n - 10) + ' belas';
    if (n < 100)
        return terbilangHuruf(Math.floor(n / 10)) + ' puluh ' + terbilangHuruf(n % 10);
    if (n < 200)
        return 'seratus ' + terbilangHuruf(n - 100);
    if (n < 1000)
        return terbilangHuruf(Math.floor(n / 100)) + ' ratus ' + terbilangHuruf(n % 100);
    if (n < 2000)
        return 'seribu ' + terbilangHuruf(n - 1000);
    if (n < 1000000)
        return terbilangHuruf(Math.floor(n / 1000)) + ' ribu ' + terbilangHuruf(n % 1000);
    return n.toString();
}
...
export function autoIsiTeksBA(dateStr) {
    if (!dateStr)
        return;
    const d = new Date(dateStr + 'T12:00:00');
    const days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
    const elHari = document.getElementById('val_hari_BA');
    if (elHari)
        elHari.value = days[d.getDay()].toLowerCase();
    const elTeks = document.getElementById('val_teks_tgl_BA');
    if (elTeks)
        elTeks.value = tanggalKeTeks(dateStr).toLowerCase();
}
