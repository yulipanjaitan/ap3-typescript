...
export function updateHariSBP() {
    let tglInput = document.getElementById('Tanggal_SBP');
    if (tglInput && tglInput.value) {
        let d = new Date(tglInput.value + 'T12:00:00');
        let days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
        let hariEl = document.getElementById('Hari_SBP');
        if (hariEl)
            hariEl.value = days[d.getDay()];
    }
}
export function updateLiveCodePreviews() {
    let valLP = document.getElementById('NoLP_LP_1')?.value.trim() || '';
    let valSBP = document.getElementById('Nomor_SBP')?.value.trim() || '';
    let valSPRIN = document.getElementById('No_SPRIN_Indak')?.value.trim() || '';
    let statusPenangkapan = document.getElementById('Status_Penangkapan_Select')?.value || '-';
...
export function updateValCodePreviews() {
    let valLP = document.getElementById('val_input_LP')?.value.trim() || '';
    let valSBP = document.getElementById('val_input_SBP')?.value.trim() || '';
...
export function generateUraianBarangOtomatis() {
    let jml = document.getElementById('Jumlah_Koli')?.value.trim() || '';
    let satuan = document.getElementById('Jenis_Koli')?.value.trim() || '';
    let barang = document.getElementById('auto_barang')?.value.trim() || '';
    let akhiranTipe = document.getElementById('auto_akhiran_select')?.value || '';
    let kustomText = document.getElementById('auto_kustom_text')?.value.trim() || '';
...
export function toggleJenisDokumen(val) {
    ...
    let dokPemberitahuan = document.getElementById('Dokumen_Pemberitahuan');
...
export function syncDokumenPemberitahuan() {
    let selEl = document.getElementById('Jenis_Dok_Pemberitahuan_Select');
    let dokPemberitahuan = document.getElementById('Dokumen_Pemberitahuan');
    let manualEl = document.getElementById('Jenis_Dok_Pemberitahuan_Manual');
    let nomorEl = document.getElementById('Nomor_Dok_Pemberitahuan');
...
export function handleJenisDokPelengkapChange(val) {
    ...
    let dokPelengkap = document.getElementById('Dokumen_Pelengkap');
    if (val === '-') {
        if (manualInput) {
            manualInput.style.display = 'none';
            manualInput.value = '';
...
export function syncDokumenPelengkap() {
    let selEl = document.getElementById('Jenis_Dok_Pelengkap_Select');
    let dokPelengkap = document.getElementById('Dokumen_Pelengkap');
    let manualEl = document.getElementById('Jenis_Dok_Pelengkap_Manual');
    let nomorEl = document.getElementById('Nomor_Dok_Pelengkap');
    let tglEl = document.getElementById('Tanggal_Dok_Pelengkap');
    if (!selEl || !dokPelengkap)
        return;
    let sel = selEl.value;
    if (sel === '-') {
        dokPelengkap.value = '-';
    ...
    if (jenis)
        parts.push(jenis);
    if (nomor)
        parts.push(`nomor ${nomor}`);
    if (tglIndo && tglIndo !== '-')
        parts.push(`tanggal ${tglIndo}`);
...
export function renderKantorDropdown() {
    ...
    let kantorInput = document.getElementById('Kantor');
    if (select) {
...
export function getFilteredDataForUser() {
    ...
    if (store.currentUser.role === 'Admin' || store.currentUser.role === 'Kepala Seksi Penyidikan') {
        ...
        return store.databasePerkara.filter((rec) => rec.disposisiPetugas && store.currentUser?.nama && rec.disposisiPetugas.toLowerCase() === store.currentUser.nama.toLowerCase());
    }
}
export function updateDashboardStats() {
    ...
    if (stLP)
        stLP.textContent = String(filteredData.length);
    let totalKerugian = 0;
    let cntBDN = 0;
    let cntSPSA = 0;
    let cntBAST = 0;
    ...
    filteredData.forEach((rec) => {
        ...
        if (/dikuasai negara|bdn/i.test(peny))
            cntBDN++;
        else if (/spsa|denda/i.test(peny))
            cntSPSA++;
        else if (/kembalikan ke pemilik|pemilik/i.test(peny))
            cntBAST++;
        else if (/serah terima.*instansi|limpah/i.test(peny))
            cntLimpah++;
        else if (peny.trim() !== '' && peny !== '-')
            cntOther++;
    });
    if (stK)
        stK.textContent = 'Rp ' + totalKerugian.toLocaleString('id-ID');
    let elBdn = document.getElementById('tl_bdn_cnt');
    if (elBdn)
        elBdn.textContent = String(cntBDN);
    let elSpsa = document.getElementById('tl_spsa_cnt');
    if (elSpsa)
        elSpsa.textContent = String(cntSPSA);
    let elBast = document.getElementById('tl_bast_cnt');
    if (elBast)
        elBast.textContent = String(cntBAST);
    let elLimpah = document.getElementById('tl_limpah_cnt');
    if (elLimpah)
        elLimpah.textContent = String(cntLimpah);
    let elOther = document.getElementById('tl_other_cnt');
    if (elOther)
        elOther.textContent = String(cntOther);
    if (filteredData.length > 0) {
        let lastRec = filteredData[filteredData.length - 1];
        if (valSummary)
            valSummary.textContent = lastRec.Penyelesaian_Perkara || '-';
        if (valStatusAlur)
            valStatusAlur.textContent = lastRec.statusAlur || 'DRAFT';
        if (valDisposisi)
            valDisposisi.textContent = lastRec.disposisiPetugas || '-';
    }
    else {
        if (valSummary)
            valSummary.textContent = '-';
        if (valStatusAlur)
            valStatusAlur.textContent = 'DRAFT / BELUM DIVALIDASI';
        if (valDisposisi)
            valDisposisi.textContent = '-';
    }
}
...
export function performLPValidation() {
    ...
    if (statusLabel)
        statusLabel.textContent = `TERVALIDASI: LP-${rec.NoLP_LP_1 || '-'} / SBP-${rec.Nomor_SBP || '-'}`;
...
export function loadRecordToCurrentView(rec) {
    ...
    if (rawIdentitas.includes(' : ')) {
        ...
        let jenisIdentitasSelect = document.getElementById('Jenis_Identitas_Select');
        if (jenisIdentitasSelect)
            jenisIdentitasSelect.value = parts[0];
        setSafe('Nomor_Identitas_Input', parts[1]);
    }
    else {
        let jenisIdentitasSelect = document.getElementById('Jenis_Identitas_Select');
        if (jenisIdentitasSelect)
            jenisIdentitasSelect.value = 'NIK';
    ...
    if (dokPabVal.toLowerCase().includes('tanpa dokumen') || !dokPabVal || dokPabVal === '-') {
        if (jenisDokSelect)
            jenisDokSelect.value = 'Tanpa Dokumen';
        if (nomorDokInput) {
            nomorDokInput.value = '';
    ...
    if (selPelengkap) {
        ...
        if (selPelengkap.value === '-') {
            if (manualPelengkap)
                manualPelengkap.style.display = 'none';
            if (noPelengkap)
                noPelengkap.disabled = true;
            if (tglPelengkap)
                tglPelengkap.disabled = true;
        }
        else if (selPelengkap.value === 'LAINNYA') {
            if (manualPelengkap)
                manualPelengkap.style.display = 'block';
            if (noPelengkap)
                noPelengkap.disabled = false;
            if (tglPelengkap)
                tglPelengkap.disabled = false;
        }
        else {
            if (manualPelengkap)
                manualPelengkap.style.display = 'none';
...
export function renderPerkaraTable() {
    ...
    tbody.innerHTML = filtered.map((rec, i) => {
        ...
        if (rec.statusAlur === 'DIUSULKAN')
            statusBadge = `<span class="badge-status badge-proposed">DIUSULKAN</span>`;
        if (rec.statusAlur && rec.statusAlur.includes('DIVALIDASI'))
            statusBadge = `<span class="badge-status badge-ready">DIVALIDASI</span>`;
...
export function addNewPerkaraModal() {
    ...
    let jenisKoliEl = document.getElementById('Jenis_Koli');
    if (jenisKoliEl)
        jenisKoliEl.style.display = 'none';
    let autoKustomEl = document.getElementById('auto_kustom_container');
    if (autoKustomEl)
        autoKustomEl.style.display = 'none';
    let asalPerkaraEl = document.getElementById('Asal_Perkara');
    if (asalPerkaraEl)
        asalPerkaraEl.style.display = 'none';
    let pekerjaanEl = document.getElementById('Pekerjaan');
    if (pekerjaanEl)
        pekerjaanEl.style.display = 'none';
    let ukuranKontainerEl = document.getElementById('Ukuran_Kontainer');
    if (ukuranKontainerEl)
        ukuranKontainerEl.style.display = 'none';
    let jenisDokManualEl = document.getElementById('Jenis_Dok_Pemberitahuan_Manual');
    if (jenisDokManualEl)
        jenisDokManualEl.style.display = 'none';
    let nomorDokInput = document.getElementById('Nomor_Dok_Pemberitahuan');
    if (nomorDokInput)
        nomorDokInput.disabled = true;
    let noSprinIndak = document.getElementById('No_SPRIN_Indak');
    if (noSprinIndak)
        noSprinIndak.value = '';
    let tglSprinIndak = document.getElementById('Tanggal_SPRIN_Indak');
    if (tglSprinIndak)
        tglSprinIndak.value = '';
    let jenisPelengkapSel = document.getElementById('Jenis_Dok_Pelengkap_Select');
    if (jenisPelengkapSel)
        jenisPelengkapSel.value = '-';
    let jenisPelengkapMan = document.getElementById('Jenis_Dok_Pelengkap_Manual');
    if (jenisPelengkapMan) {
        jenisPelengkapMan.value = '';
        jenisPelengkapMan.style.display = 'none';
    }
    let nomorPelengkap = document.getElementById('Nomor_Dok_Pelengkap');
    if (nomorPelengkap) {
    ...
    if (dokPelengkap)
        dokPelengkap.value = '-';
    let kantorSelect = document.getElementById('Kantor_Select');
    let kantorInput = document.getElementById('Kantor');
    ...
    if (perkaraModal)
        perkaraModal.classList.add('active');
}
export function editPerkara(idx) {
    ...
    document.getElementById('editingRecordIndex').value = String(realIdx >= 0 ? realIdx : idx);
...
export function executePerkaraSubmission() {
    try {
        ...
        let rec = {
            ...
            No_SPRIN_CACAH: document.getElementById('No_SPRIN_CACAH')?.value.trim() || '',
            Tanggal_SPRIN_CACAH: document.getElementById('Tanggal_SPRIN_CACAH')?.value || '',
            Nomor_BA: document.getElementById('Nomor_BA')?.value.trim() || '',
            Tanggal_BA: document.getElementById('Tanggal_BA')?.value || '',
            Nomor_LHP: document.getElementById('Nomor_LHP')?.value.trim() || '',
            Tanggal_LHP: document.getElementById('Tanggal_LHP')?.value || '',
