...
export async function initUserAccounts() {
    ...
    let hasAdminSimple = store.userAccounts.some((u) => u.email && u.email.toLowerCase() === "admin");
    if (!hasAdminSimple) {
...
export async function handleLoginSubmit(e) {
    ...
    const user = store.userAccounts.find((u) => u.email && u.email.toLowerCase() === email.toLowerCase() &&
...
export function handleRegisterSubmit(e) {
    e.preventDefault();
    let namaEl = document.getElementById('reg_nama');
    let emailEl = document.getElementById('reg_email');
    let roleEl = document.getElementById('reg_role');
    let passEl = document.getElementById('reg_password');
    ...
    if (store.userAccounts.some((u) => u.email && u.email.toLowerCase() === email)) {
...
export function deleteUserAccount(index) {
    ...
    showToastConfirm("KONFIRMASI HAPUS AKUN", `Apakah Anda yakin ingin menghapus akun user "${user.nama}" (${user.email})?`, () => {
...
export function populateDisposisiUserDropdown() {
    ...
    select.innerHTML = `<option value="-">- Pilih Penyidik / Peneliti Tujuan -</option>` +
        penyidikList.map((u) => `<option value="${escText(u.nama)}">${escText(u.nama)} (${escText(u.email)})</option>`).join('');
}
